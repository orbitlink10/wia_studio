...
<body class="@yield('body_class')">
    <header class="site-header big-header" id="siteHeader">
        ...
        <a class="brand" href="{{ route('home') }}" aria-label="WIA Studio home">
            <span class="brand-mark"><img src="{{ asset('assets/img/wia-logo.svg') }}" alt=""></span>
            <span>
        ...
        <nav class="site-nav" id="siteNav">
            <a href="{{ route('projects.index') }}" data-category-trigger="architecture" data-wia-filter-nav="architecture">Architecture</a>
            ...
            @else
                <a class="admin-nav-access" href="{{ route('admin.signup') }}">Admin sign up</a>
            @endauth
        </nav>
    </header>
    ...
    <footer class="footer">
        ...
        <div class="footer-grid">
            <div>
                <span>Contact</span>
                <a href="mailto:{{ $siteProfile->contact_email ?? 'studio@wia.com' }}">{{ $siteProfile->contact_email ?? 'studio@wia.com' }}</a>
                <a href="tel:{{ preg_replace('/[^0-9+]/', '', $siteProfile->phone_number ?? '+254700000000') }}">{{ $siteProfile->phone_number ?? '+254 700 000 000' }}</a>
                <a href="https://www.wia.com">www.wia.com</a>
                <p>Nairobi, Kenya</p>
            </div>
            <div>
                <span>Routes</span>
                <a href="{{ route('projects.index') }}">Project archive</a>
                <a href="{{ route('about.index') }}">About</a>
            </div>
            <div>
                <span>Social</span>
